...
public class TextureCopier {
  ...
  public String getActualTextureFileName(String sourceFileName) {
    ...
    boolean needResize = !MathUtil.isPowerOfTwo(imageSize[0]) || !MathUtil.isPowerOfTwo(imageSize[1]) ||
      imageSize[0] < 64 || imageSize[1] < 64;
    imageSize[0] = Math.max(MathUtil.getClosestPowerOfTwo(imageSize[0]), 64);
    imageSize[1] = Math.max(MathUtil.getClosestPowerOfTwo(imageSize[1]), 64);
    String line = null;
    if (needResize) {
