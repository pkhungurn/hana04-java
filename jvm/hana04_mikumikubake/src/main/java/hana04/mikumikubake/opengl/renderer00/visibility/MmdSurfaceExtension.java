...
public final class MmdSurfaceExtension {
  private MmdSurfaceExtension() {
    // NO-OP
  }

  public static class PositionMapRenderer_ extends AbstractMmdSurfaceRender00Receiver implements PositionMapRenderer {
    private final Wrapped<ProgramAsset> programAsset;

    @HanaDeclareExtension(
        extensibleClass = MmdSurface.class,
        extensionClass = PositionMapRenderer.class)
    public PositionMapRenderer_(MmdSurface mesh, HanaUnwrapper unwrapper, GlWrapper glWrapper) {
      ...
      this.programAsset = new Cached<>(CacheKey.builder()
          ...
          .addStringPart(Constants.WORLD_POSITION_FRAG_RESOURCE_NAME)
          .build());
    }
  ...
  public static class VisibilityMapRenderer_ extends AbstractMmdSurfaceRender00Receiver implements VisibilityMapRenderer {
    private final Wrapped<ProgramAsset> programAsset;

    @HanaDeclareExtension(
        extensibleClass = MmdSurface.class,
        extensionClass = VisibilityMapRenderer.class)
    public VisibilityMapRenderer_(MmdSurface mesh, HanaUnwrapper unwrapper, GlWrapper glWrapper) {
      super(mesh, unwrapper, glWrapper);
      this.programAsset = new Cached<>(CacheKey.builder()
          .protocol(ResourceProgramAssetExtensions.PROTOCOL)
          .addStringPart(Constants.MMD_POSE_MESH_START_TO_END_VERT_RESOURCE_NAME)
          .addStringPart(Constants.VISIBILITY_FRAG_RESOURCE_NAME)
          .build());
    }
