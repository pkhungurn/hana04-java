...
uniform sampler2DRect mesh_boneTransform_start;
uniform sampler2DRect mesh_morphWeight_start;
uniform sampler2DRect mesh_boneTransform;
uniform sampler2DRect mesh_morphWeight;

out vec3 geom_normal;
out vec3 geom_startPosition;
out vec3 geom_endPosition;
...
void main() {
    VertexConfig posedVcStart = getPosedVertexConfig(mesh_boneTransform_start, mesh_morphWeight_start);
    VertexConfig posedVcEnd = getPosedVertexConfig(mesh_boneTransform, mesh_morphWeight);

    vec4 worldPositionStart = sys_modelMatrix * vec4(posedVcStart.p, 1);
    vec4 worldPositionEnd = sys_modelMatrix * vec4(posedVcEnd.p, 1);
    vec4 cameraPosition = sys_viewMatrix * vec4(worldPositionEnd);
    vec4 clipPosition = sys_projectionMatrix * vec4(cameraPosition);

    geom_normal = normalize((sys_normalMatrix * vec4(posedVcEnd.n, 1)).xyz);
    geom_startPosition = worldPositionStart.xyz;
    geom_endPosition = worldPositionEnd.xyz;
